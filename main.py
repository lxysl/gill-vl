...
def parse_args(args):
  ...
  parser.add_argument('--num-emb2-tokens', default=256, type=int, metavar='N', help='Number of Embedding 2 token to use for generation.')
  ...
  text_fc_modes = ['linear', 'gill_mapper', 'gill_mapper_hunyuan', 'gill_mapper_kolors']
...
def main_worker(gpu, ngpus_per_node, args):
  ...
  model_args.num_emb2_tokens = args.num_emb2_tokens
  ...
  if args.text_fc_mode in ["gill_mapper_hunyuan", "gill_mapper_kolors"]:
    collate_fn = data.custom_images_collate_fn_2
  else:
    collate_fn = data.custom_images_collate_fn
...
def train(train_loader, model, tokenizer, criterion, optimizer, epoch, scheduler, args):
  ...
  for i, batch in enumerate(train_loader):
    if args.text_fc_mode in ["gill_mapper_hunyuan", "gill_mapper_kolors"]:
      _, raw_images, images, image_grid_thw, caption_images, cap_token_ids, cap_labels, cap_start_id, cap_end_id, gen_token_ids, gen_labels, gen_start_id, gen_end_id, emb, emb_2 = batch
    else:
      _, raw_images, images, image_grid_thw, caption_images, cap_token_ids, cap_labels, cap_start_id, cap_end_id, gen_token_ids, gen_labels, gen_start_id, gen_end_id, emb = batch
    ...
    if torch.cuda.is_available():
      ...
      emb = emb.cuda(args.gpu, non_blocking=True)
      if args.text_fc_mode in ["gill_mapper_hunyuan", "gill_mapper_kolors"]:
    ...
    for model_mode in model_modes:
      ...
      if model_mode == 'retrieval':
        ...
        if args.num_tokens != 0 and args.num_clip_tokens != args.num_tokens:
          ...
          assert last_embedding.shape == emb.shape, (last_embedding.shape == emb.shape)

        if args.text_fc_mode in ["gill_mapper_hunyuan", "gill_mapper_kolors"]:
          image_loss = losses_utils.l2_loss(emb, last_embedding)  # (N,)
